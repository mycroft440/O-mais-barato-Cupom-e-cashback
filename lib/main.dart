...
class Product {
  ...
  bool get isCheapestCompared =>
      comparisonPrice != null && comparedListings > 1;
...
class PreferenceEngine extends ChangeNotifier {
  ...
  Future<void> load() async {
    ...
    for (final raw in prefs.getStringList('wishlist') ?? const []) {
      try {
        _wishlist.add(
          WishItem.fromJson(jsonDecode(raw) as Map<String, dynamic>),
        );
      } catch (_) {}
    }
    notifyListeners();
  ...
  double score(Product product) {
    var score = product.savingsVsPeersPercent * .4;
...
class MarketComparator {
  ...
  static List<Product> dealFeed(
    ...
    // Não existe corte percentual: qualquer diferença é válida.
    // O parâmetro permanece apenas por compatibilidade com versões antigas.
    return cheapestPerProduct(listings)
        .where((p) => p.comparedListings >= 2)
        .toList();
  }
}
...
class ProductSearchService {
  ...
  Future<List<Product>> search(String query) async {
    ...
    if (_apiBaseUrl.isNotEmpty) {
      try {
        final uri = Uri.parse('$_apiBaseUrl/search').replace(
          queryParameters: {'q': cleaned, 'limit': '20'},
        );
        final response =
    ...
    final matches = demoListings.where((p) {
      final haystack =
          '${p.name} ${p.category} ${p.brand} ${p.tags.join(' ')}'
              .toLowerCase();
      return haystack.contains(q) ||
          (q == 'tenis' && haystack.contains('tênis'));
    });
...
const demoListings = <Product>[
  ...
  Product(
    ...
    originalPrice: 499,
    tags: ['tênis', 'corrida'],
    catalogProductId: 'olympikus-corre-4',
  ...
  Product(
    ...
    price: 79,
    originalPrice: 109,
    tags: ['creatina', 'academia'],
    catalogProductId: 'creatina-300',
    popularityPosition: 5,
  ),
  Product(
    id: 'creatina-b',
    name: 'Creatina 300 g',
...
class DealsPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final cheapest = MarketComparator.dealFeed(demoListings);
    return AnimatedBuilder(
      animation: engine,
      builder: (context, _) {
        final deals = engine.recommend(cheapest);
        return ListView(
          children: [
            const PageTitle(
              'Mais baratos agora',
              'Cada cartão mostra o menor preço encontrado para aquele produto. Não existe corte mínimo de desconto.',
            ),
          ...
          child: results.isEmpty && !loading
              ? const Center(
                  child: Padding(
                    ...
                    child: Text(
                      'Pesquise um produto. Exemplo: “tênis”.',
              ...
              : ListView(
                  children: results
                      .map((p) => ProductCard(product: p, engine: widget.engine))
                      .toList(),
                ),
        ),
      ],
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${product.marketplace}${product.seller == null || product.seller!.isEmpty ? '' : ' • ${product.seller}'}',
                      style: const TextStyle(
                        ...
                        children: [
                          ...
                          if (product.savingsVsPeersPercent > 0)
                            Chip(
                              visualDensity: VisualDensity.compact,
                              label: Text(
                                '${product.savingsVsPeersPercent.toStringAsFixed(product.savingsVsPeersPercent < 10 ? 1 : 0)}% abaixo da referência',
                              ),
                            ),
                        ],
                      ),
                      Text(
                        'Referência dos outros anúncios: ${money(product.comparisonPrice!)}',
                        style: const TextStyle(
                          fontSize: 12,
                          color: Colors.black54,
                        ),
                    ...
                    if (product.couponCode != null) ...[
                      const SizedBox(height: 6),
                      Text(
                        'Cupom ${product.couponCode} • ${money(product.couponDiscount)} OFF',
                      ),
                    ],
                  ],
                ),
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            '${product.marketplace} • ${product.brand}${product.seller == null || product.seller!.isEmpty ? '' : ' • ${product.seller}'}',
          ),
          const SizedBox(height: 18),
          Text(
            'R$ ${product.finalPrice.toStringAsFixed(2).replaceAll('.', ',')}',
            style: Theme.of(context)
                .textTheme
                .displaySmall
                ?.copyWith(fontWeight: FontWeight.w900),
          ...
          if (product.isCheapestCompared)
            Card(
              child: ListTile(
                ...
                subtitle: Text(
                  product.savingsVsPeersPercent > 0
                      ? '${product.savingsVsPeersPercent.toStringAsFixed(1)}% abaixo da referência dos outros anúncios (R$ ${product.comparisonPrice!.toStringAsFixed(2).replaceAll('.', ',')}).'
                      : 'Há empate no menor preço. Referência dos demais: R$ ${product.comparisonPrice!.toStringAsFixed(2).replaceAll('.', ',')}.',
                ),
              ),
            ),
          if (product.couponCode != null)
          ...
          children: [
            ...
            Wrap(
              ...
              children: [1, 3, 6, 12]
                  .map(
                    (m) => ChoiceChip(
                      label: Text('$m ${m == 1 ? 'mês' : 'meses'}'),
                      selected: months == m,
                      onSelected: (_) =>
                          setDialogState(() => months = m),
                    ),
                  )
                  .toList(),
            ),
      ...
      builder: (context, _) {
        ...
        return Column(
          children: [
            const PageTitle(
              'Quero comprar',
              'Acompanhe um produto e receba alertas quando aparecer um preço menor.',
            ),
            Expanded(
              child: items.isEmpty
                  ...
                  : ListView(
                      children: items.map((item) {
                        ...
                        return ListTile(
                          ...
                          subtitle: item.targetPrice == null
                              ? const Text('Sem preço-alvo')
                              : Text(
                                  'Alvo: R$ ${item.targetPrice!.toStringAsFixed(2).replaceAll('.', ',')}',
                                ),
                          trailing: IconButton(
                            onPressed: () =>
                                engine.removeWish(item.productId),
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final coupons = engine.recommend(
          demoProducts.where((p) => p.couponCode != null).toList(),
        );
        return ListView(
          children: [
            const PageTitle(
      ...
      builder: (context, _) {
        ...
        return ListView(
          children: [
            const PageTitle(
              'Suas preferências',
              'O app aprende com suas pesquisas, cliques e produtos monitorados.',
            ),
